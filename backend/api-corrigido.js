...
async function fetchRealCryptoData() {
    ...
    if (now - lastUpdate < UPDATE_INTERVAL && Object.keys(marketCache).length > 0) {
        return marketCache;
    }
    ...
    try {
        ...
        const response = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,usd-coin,aave,uniswap&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true&include_market_cap=true');
...
app.get('/api/portfolio/multichain/:address', async(req, res) => {
    try {
        ...
        // Emite dados do portfólio via WebSocket
